...
@bp.route('/api/unidades_de_saude', methods=['GET', 'POST'])  # Certifique-se de que o endpoint está correto
...
@bp.route('/api/unidades_de_saude/id/<int:id>', methods=['GET'])  # Certifique-se de que o endpoint está correto
@swag_from('unidades.yml')
def listar_unidade_de_saude(id):
    unidade_por_id = UnidadeDeSaude.query.filter_by(id=id).first()
    return jsonify(unidade_por_id.to_dict()) if unidade_por_id else abort(404, "Unidade de saúde não encontrada.") #Alterado
